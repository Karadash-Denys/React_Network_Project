...
const ProfileInfo: React.FC<PropsType> = ({
    ...
    const onMainFileSelected = (e: ChangeEvent<HTMLInputElement>) => {
        if (e.target.files?.length) {
            // add e.target.files && e.target.files?.length
            savePhoto(e.target.files[0])
        }
    }

    const onSubmitLogin = async (formData: ProfileType) => {
        await saveProfile(formData).then(() => setEditMode(false))
    }
    ...
    return (
        <div className={s.img_block}>
            ...
            <div className={s.userProfile}>
                ...
                <div>
                    {isOwner && (
                        <input type="file" onChange={onMainFileSelected} />
                    )}
                </div>
                {editMode ? (
